...
function applyMilitaryTerminology() {
    ...
    searchInputs.forEach(input => {
        if (input.placeholder.toLowerCase().includes('what do you want to play')) {
...
function checkAndMuteAd() {
    ...
    if (isAd && !isAdCurrentlyPlaying) {
        ...
        const randomAudio = militaryAudios[Math.floor(Math.random() * militaryAudios.length)];
        adAudioPlayer = new Audio(randomAudio);
        adAudioPlayer.volume = 0.5;
        adAudioPlayer.loop = true;
        adAudioPlayer.play().catch(e => console.log("Audio play blocked by browser:", e));

        showAdBlockedWarning(true);

    } else if (!isAd && isAdCurrentlyPlaying) {
        console.log("ALL CLEAR. Resuming transmission.");
...
console.log("MILITARY OVERRIDE INITIALIZED. COMM MUTE ACTIVE.");